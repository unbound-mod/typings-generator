...
export function error(...args: string[]): void {
	return console.error('»', ...args.map(arg => colorize(typeof arg === 'string' ? arg : inspect(arg), 'red')));
}

export function success(...args: string[]): void {
	return console.log('»', ...args.map(arg => colorize(typeof arg === 'string' ? arg : inspect(arg), 'green')));
}

export function warn(...args: string[]): void {
	return console.warn('»', ...args.map(arg => colorize(typeof arg === 'string' ? arg : inspect(arg), 'yellow')));
}

export function debug(...args: string[]): void {
	return console.debug('»', ...args.map(arg => colorize(typeof arg === 'string' ? arg : inspect(arg), 'gray')));
}
